...
class _AceleroGameState extends State<AceleroGame> {
  ...
  DateTime _lastShakeTime = DateTime.now().subtract(Duration(seconds: 1));
  int _coins = 0;
  bool _canShake = true;
  ...
  void _loadPrefs() async {
    ...
    setState(() {
      _lastShakeTime = DateTime.fromMillisecondsSinceEpoch(
          _prefs.getInt('lastShakeTime') ?? DateTime.now().subtract(Duration(seconds: 1)).millisecondsSinceEpoch);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          Image.asset(
            'assets/images/background_games.png',
            fit: BoxFit.cover,
